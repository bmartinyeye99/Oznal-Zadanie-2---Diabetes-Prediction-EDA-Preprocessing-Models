...
ggplot(data = correlation_df_long, aes(x = row, y = variable, fill = value)) +
  geom_tile(color = "white") + # Adjust width and height of cells
  geom_text(aes(label = round(value, 2)), color = "black", size = 2.2) + # Adjust text size
...
for (col in cols) {
  ...
  plot <- data %>%
    ...
    theme_minimal() + 
    labs(x = col, y = "Count", title = 'Diabetes in other columns') +
    theme(plot.title = element_text(hjust = 0.5, size = 20, face = 'bold')) +
    scale_fill_manual('Diabetes', values = c('coral2', 'steelblue'))
...
Na t7chtoch grafoch vidíme ditribúciu Diabete medzi ostatnými binárnými hodnotami.


```{r}
barplot <- ggplot(data, aes(x = factor(Diabetes), fill = factor(Diabetes))) +
  geom_bar() +
  scale_fill_manual(values = c("blue", "red")) +
  labs(x = "Diabetes", y = "Count", title = "Distribution of Diabetes") +
  theme_minimal()
