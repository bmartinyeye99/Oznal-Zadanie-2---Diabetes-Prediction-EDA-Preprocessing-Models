...
random_samples$Diabetes <- ifelse(random_samples$Diabetes==0,"No","Yes")
...
auc_linear <- max(svm_model_linear$results$ROC)
auc_radial <- max(svm_model_radial$results$ROC)
auc_poly <- max(svm_model_poly$results$ROC)

max_auc <- max(auc_linear, auc_radial, auc_poly)
best_method <- ifelse(max_auc == auc_linear, "svmLinear",
                      ifelse(max_auc == auc_radial, "svmRadialSigma", "svmPoly"))

# Print the results
cat("Best performing model:", best_method, "\n")
cat("AUC:", max_auc, "\n")
