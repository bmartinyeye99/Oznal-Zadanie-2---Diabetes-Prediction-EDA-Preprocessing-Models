...
trainIndex <- createDataPartition(data_balanced$Diabetes, p = 0.9, list = FALSE)
...
svm_model <- train(formula = formula, 
                   data = trainData, 
                   method = "svmRadialSigma", # Specify the default method
                   trControl = ctrl,
                   tuneGrid = expand.grid(
                     .sigma = seq(0.005, 0.2, length = 10),
                     .C = seq(0.1, 10)
                   ),
...
```{r}
svm_model$bestTune
svm_model$results[svm_model$results$Accuracy== max(svm_model$results$Accuracy), ]
```
